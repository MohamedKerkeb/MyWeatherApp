...
function App() {
  ...
  // useEffect(  () => {
    ...
    const URL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&lang=fr&appid=${APIKEY}`
      ...
      }).catch(err => {console.log(err); setLoading(false); setError(true) })
    };
  //   getWeather()
  //  }, [])
  ...
  return (
    <div className="AppWrapper">
      {console.log(weather)}
      <Header color={mapping.colors  [
        error ? "error" : weather.description
      ]}  onClickHandler={() => tryAgain()}/>
      <main className='AppMain'>
        <SearchBar value={search} onChangeHandler={searchBar} onClickHandler={() => getWeather()} error={error}/>
        <Card>
          {cardContent}
        </Card>
      </main>
